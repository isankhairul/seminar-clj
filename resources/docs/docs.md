...
The `home-routes` handler in the `seminar.routes.home` namespace
...
The `home-page` function will in turn call the `seminar.layout/render` function
...
The routes are aggregated and wrapped with middleware in the `seminar.handler` namespace:
...
Request middleware functions are located under the `seminar.middleware` namespace.

This namespace is reserved for any custom middleware for the application. Some default middleware is
already defined here. The middleware is assembled in the `wrap-base` function.

Middleware used for development is placed in the `seminar.dev-middleware` namespace found in
